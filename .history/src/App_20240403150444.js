...
function App() {
  ...
  const handleCountryChange= as(e)=>{
    console.log(e.target.value)
    setSelectedCountry(e.target.value)

    try{
      const data = await fetch(`https://crio-location-https://crio-location-selector.onrender.com/country=${e.target.value}/statesselector.onrender.com/countries`)
      const res = await data.json();
      setCountries(res)
    }
    catch(e){
